...
def accuracy_calc(output, target, label_size, greaterthan=0.1, topk=10):
    ...
    for jj in range(num_query):
        ...
        prediction_index = torch.mul(prediction[1] + 1, filter.to(prediction[1].dtype).to(prediction[1].device))



        extend_eye_mat = torch.cat(
            (torch.zeros(1, label_size, device=output.device), torch.eye(label_size, device=output.device)), 0)

        prediction_label         = extend_eye_mat[prediction_index.view(-1)].view(-1, topk_size, label_size).sum(dim=1)
 
        correct_prediction_label = (target[jj].byte().to(prediction_label.device) & prediction_label.byte())
